...
let client = null;
let config = null;   // ✅ 新增：保存 config.json
let historyFetcher = null; // ✅ 新增：延后创建
...
document.getElementById('btnConn').onclick = () => {
  ...
  client = connectMqtt({
    ...
    onMessage: (t, payload) => {
      console.log('[app.js] raw topic:', t);
      console.log('[app.js] raw payload:', payload?.toString?.());

      const text = payload.toString();
      const obj = safeJson(text);

      console.log('[app.js] parsed safeJson:', obj);
      ...
      // if (!parsed) return;
      const parsed = parseTelemetry(obj);
      console.log('[app.js] parseTelemetry result:', parsed);
      if (!parsed) return;
...
async function init() {
  try {
    ...
    // 兼容两种配置结构：
    // 1) 旧版：cfg.host / cfg.port / cfg.topic
    // 2) 新版：cfg.hive.host / cfg.hive.port / cfg.hive.topic
    const hiveCfg = cfg.hive || cfg;
    if (hiveCfg.username && !el.user.value) el.user.value = hiveCfg.username;
    if (hiveCfg.password && !el.pass.value) el.pass.value = hiveCfg.password;

    el.host.value = hiveCfg.host ?? el.host.value;
    el.port.value = hiveCfg.port ?? el.port.value;
    el.path.value = hiveCfg.path ?? el.path.value;
    el.topic.value = hiveCfg.topic ?? el.topic.value;
    el.maxPoints.value = hiveCfg.maxPoints ?? el.maxPoints.value;
    OFFLINE_MS = hiveCfg.offlineMs ?? OFFLINE_MS;

    config = cfg;
    historyFetcher = createHistoryFetcher({
    ...
    const lastUser = localStorage.getItem('viewer_user');
    if (!el.user.value && lastUser) el.user.value = lastUser;
    el.user.addEventListener('change', () => localStorage.setItem('viewer_user', el.user.value));
  } catch (e) {
