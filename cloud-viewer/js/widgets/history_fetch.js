...
export function createHistoryFetcher({
  ...
  function cleanup(client) {
    ...
    if (activeReplyTo) {
      try { client.unsubscribe(activeReplyTo); } catch {}
      activeReplyTo = null;
    }
  }
  ...
  async function fetchCsv() {
    ...
    const handler = (t, payload) => {
      ...
      if (msg.type === "end") {
        ...
        try {
          if (chunkArr.some(x => typeof x !== "string")) {
            throw new Error("存在缺失分片");
          }
    ...
    console.log("[RPC web request topic]", "cloud/uav/rpc/history/request");
    console.log("[RPC web request msg]", reqMsg);

    client.publish("cloud/uav/rpc/history/request", JSON.stringify(reqMsg));
  }
  ...
  async function listDates() {
    ...
    const handler = (t, payload) => {
      console.log("[RPC web reply raw]", t, payload.toString());
      if (t !== replyTo) return;
      ...
      if (msg.type === "error") {
        setStatus(`失败：${msg.message}（如果你还没实现 list_dates RPC，这是正常的）`);
        cleanup(client);
        return;
      }
